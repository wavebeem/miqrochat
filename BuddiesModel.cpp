...
QVariant BuddiesModel::data(const QModelIndex &index, int role) const {
    ...
    if (role == Qt::FontRole && item && item->group()) {
        QFont font;
        font.setBold(true);
...
void BuddiesModel::setupModelData(const QStringList &list, BuddyItem *root) {
    ...
    foreach (const QString &line, list) {
        if (line.startsWith("#")) {
            parent = root;
            Group *g = new Group(line.mid(1));
            item   = new BuddyItem(g, parent);
            ...
            QString nick = items[0];
            QString id   = items[1];
            Buddy *b = new Buddy(nick, id);
            item = new BuddyItem(b, parent);
            parent->appendChild(item);
        }
    }
}
